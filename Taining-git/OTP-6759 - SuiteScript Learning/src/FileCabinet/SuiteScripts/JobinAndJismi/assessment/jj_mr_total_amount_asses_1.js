    ...
    (email, record, runtime, search) => {
        ...
        const getInputData = (inputContext) => {
            var loadSearch = search.load({
                id: 'customsearch_jj_total_sales_amount'
            });
...
try{
    ...
    log.debug('err',e)
        ...
        var totalSalesAmount = reduceContext.values.reduce(function(acc, value) {
            return acc + parseFloat(value);
        }, 0);
        log.debug('totalSalesAmount' + customerId ,totalSalesAmount)

        // Emit key-value pair with customerId as key and totalSalesAmount as value
     //   reduceContext.write(customerId, totalSalesAmount);
           reduceContext.write({
                    key: reduceContext.key ,
